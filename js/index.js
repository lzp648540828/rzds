...
(function () {
    let index  = 0;
    let nodeList = document.querySelectorAll(".carousel .images  li");

    nodeList[0].classList.add("visible");

    let slideList = document.querySelectorAll(".carousel .carousel-index li");
    for(let i = 0;i < slideList.length;i++){
        slideList[i].setAttribute("index",i.toString());
    }
    slideList[0].classList.add("active");
    ...
    function load() {
        let previous = document.querySelector("#famous-btn-previous");
        let next = document.querySelector("#famous-btn-next");
        previous.onclick = previousTip;
        next.onclick = nextTip;
        slideList.forEach((e)=>{
            e.onclick = goToIndex;;
